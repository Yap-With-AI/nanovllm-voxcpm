...
class VoxCPMEncoderConfig(BaseModel):
    """Configuration for VoxCPM local encoder."""
...
class VoxCPMDitConfig(BaseModel):
    """Configuration for VoxCPM diffusion transformer."""
...
class VoxCPMConfig(BaseModel):
    """Main configuration for VoxCPM model."""
...
class VoxCPMModel(nn.Module):
    """Main VoxCPM model for text-to-speech generation."""
    ...
    def __init__(
        ...
        if not torch.cuda.is_available():
            if torch.backends.mps.is_available():
                ...
                self.device = "cpu"
        logger.info("Running on device: %s, dtype: %s", self.device, self.config.dtype)
    ...
    def forward(
        ...
        *,
        ...
        Args:
            ...
            text_mask: Mask for text positions
            audio_feats: Audio features [B, T, P, D]
            audio_mask: Mask for audio positions
            loss_mask: Mask for loss computation
            position_ids: Position IDs (not used yet)
            labels: Stop prediction labels
            progress: Training progress (0-1)
            sample_generate: Whether to generate samples

        Returns:
            Dict with loss/diff, loss/stop, feat_gt, feat_pred
        """
        ...
        enc_outputs = (
            self.fsq_layer(enc_outputs) * audio_mask.unsqueeze(-1) + enc_outputs * text_mask.unsqueeze(-1)
        )
        ...
        if sample_generate:
            ...
            feat_pred = rearrange(
                feat_pred_seq.transpose(1, 2), "(b t) d p -> b d (t p)", b=B, p=self.patch_size
            )
    ...
    def _generate(
        ...
        if len(prompt_wav_path) == 0:
            ...
            if audio.size(1) % patch_len != 0:
                # Left padding: pad at the beginning of the audio
            ...
            if streaming:
                ...
                if retry_badcase:
                    if pred_audio_feat.shape[0] >= target_text_length * retry_badcase_ratio_threshold:
                        logger.info(
                            "Badcase detected, audio_text_ratio=%.2f, retrying...",
    ...
    def merge_prompt_cache(
        ...
        """Merge original prompt cache with newly generated content."""
    ...
    def generate_with_prompt_cache_streaming(
        ...
        """Generate audio using pre-built prompt cache with streaming."""
    ...
    def _generate_with_prompt_cache(
        ...
        """Internal generation with prompt cache."""
        if retry_badcase and streaming:
            warnings.warn(
                "Retry on bad cases is not supported in streaming mode, setting retry_badcase=False."
            )
    ...
    def inference(self, *args, **kwargs) -> Tuple[torch.Tensor, torch.Tensor]:
        """Run inference and return latents."""
    ...
    @classmethod
    def from_local(
        cls, path: str, optimize: bool = True, training: bool = False, lora_config: LoRAConfig = None
    ):
        """Load model from local checkpoint.

        Args:
            path: Path to checkpoint directory
            optimize: Whether to apply torch.compile optimizations
            training: Whether to load in training mode
            lora_config: Optional LoRA configuration

        Returns:
            Loaded VoxCPMModel instance
        """
        ...
        if os.path.exists(safetensors_path) and SAFETENSORS_AVAILABLE:
            logger.info("Loading model from safetensors: %s", safetensors_path)
            model_state_dict = load_file(safetensors_path)
        elif os.path.exists(pytorch_model_path):
            logger.info("Loading model from pytorch_model.bin: %s", pytorch_model_path)
    ...
    def load_lora_weights(self, lora_path: str, device: str = None) -> Tuple[List[str], List[str]]:
