"""MiniCPM4 transformer model implementation."""
...
def rms_layernorm(hidden: torch.Tensor, weight: torch.Tensor, eps: float) -> torch.Tensor:
...
class MiniCPMRMSNorm(nn.Module):
    """MiniCPM RMS normalization (equivalent to T5LayerNorm)."""
...
class MiniCPMLongRoPE(nn.Module):
    """MiniCPM rotary embedding with Dynamic NTK scaling."""
    ...
    def forward(self, position_ids: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """Get cos and sin embeddings for position IDs.

        Args:
            position_ids: Tensor(seq_len) or Tensor(batch_size, seq_len)
...
class MiniCPMAttention(nn.Module):
    """Multi-head attention for MiniCPM."""
    ...
    def forward(
        ...
        # there is a bug in MPS for non-contiguous tensors
...
class MiniCPMDecoderLayer(nn.Module):
    """Single decoder layer for MiniCPM."""
    ...
    def forward(
        ...
        Args:
            ...
            position_emb: Position embeddings (cos, sin)
...
class MiniCPMModel(nn.Module):
    """Transformer decoder consisting of MiniCPMDecoderLayer layers."""
    ...
    def forward(
        ...
        Returns:
            ...
            next_decoder_cache: List of (key, value) tuples per layer
        """
    ...
    def setup_cache(self, batch_size: int, max_length: int, device: torch.device, dtype: torch.dtype) -> None:
        """Setup static KV cache for inference."""
